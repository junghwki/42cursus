...
void ft_we_route(t_box *box)
{
	...
	box->we.addr = (int *)mlx_get_data_addr(box->we.ptr, &box->we.bits_per_pixel, &box->we.size_line, &box->we.endian);
	box->pars.we_flag = 1;
}
...
void ft_so_route(t_box *box)
{
	...
	box->so.addr = (int *)mlx_get_data_addr(box->so.ptr, &box->so.bits_per_pixel, &box->so.size_line, &box->so.endian);
	box->pars.so_flag = 1;
}
...
void ft_no_route(t_box *box)
{
	...
	box->no.addr = (int *)mlx_get_data_addr(box->no.ptr, &box->no.bits_per_pixel, &box->no.size_line, &box->no.endian);
	box->pars.no_flag = 1;
}
void ft_s_route(t_box *box)
{
	...
	box->s.addr = (int *)mlx_get_data_addr(box->s.ptr, &box->s.bits_per_pixel, &box->s.size_line, &box->s.endian);
	box->pars.s_flag = 1;
}